...
program.name('express').version(version, '    --version').usage('[options] [dir]').option('    --git', 'add .gitignore').option('-f, --force', 'force on non-empty directory').parse(process.argv);
...
function createApplication(name, path) {
  ...
  mkdir(path, function () {
    ...
    var pkg = {
      ...
      keywords: ['node', 'create-react-app', 'react'],
      dependencies: {
        'body-parser': '~1.18.2',
        'babel-cli': '~6.26.0',
        'babel-core': '~6.26.0',
...
//# sourceMappingURL=express-cli.js.map